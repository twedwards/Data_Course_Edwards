...
a = utah %>% group_by(Pop.Group) %>% summarize(Religious = mean(Religious))
...
b = utah %>% group_by(Pop.Group) %>% summarize(Pop_2010 = mean(Pop_2010))
...
religiosity = arrange(religiosity, desc(Pop_2010))
