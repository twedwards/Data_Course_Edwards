...
ggplot(midwest) # what do you see? Nothing. We didn't give it instructions as to the aesthetics for the plot

# give it some aesthetics to work with...
ggplot(midwest, aes(x=area, y=poptotal))  # area and poptotal are columns in 'midwest'. this says that there should be an X and Y axis
...
####The line above tells ggplot what file to plot, how to plot it (with axises), and how we want to see it(scatterplot)

# Add another geom ... a trendline:
ggplot(midwest, aes(x=area, y=poptotal)) + geom_point() + geom_smooth(method = "lm")
####The above line takes the scatter plot from above, and adds the linear regression aspect to the scatterplot.
...
# Zoom in
p + lims(x=c(0,0.1),y=c(0,1000000)) # what did this do? It gave the plot limits, and specified what the x and y limits are.
p + coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) # how is this different? adds cartesian coordinates with the assigned x and y limits
...
####This one is different because there is color added by the command color="firebrick"
ggplot(midwest, aes(x=area, y=poptotal)) + 
  geom_point(color="steelblue",size=3) + ####This adds the color steelblue to the points
...
p3 <- ggplot(midwest, aes(x=area, y=poptotal)) + 
  geom_point(aes(color=state),size=3,alpha=.5) + ####this adds an aesthetic to the points, coloring by state, and it also creates the legend
  geom_smooth(method="lm",color="firebrick") + 
  coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) + 
  labs(title="Area Vs Population", subtitle="From midwest dataset", y="Population", x="Area", caption="Midwest Demographics", color="State")
p3
...
ggplot(midwest, aes(x=poptotal)) +
  geom_histogram(aes(fill=state),bins = 10) +
  facet_wrap(~state) +
  labs(x="Population", y="Count", title="Population by State", color="State")

ggplot(midwest, aes(x=poptotal)) +
  geom_density(aes(fill=state), alpha=.25) + ####geom_density is a smoothline version of a histogram
...
####Aesthetics are what will be plotted. 
...
p4 + facet_wrap(~ state) #### facet_wrap only works on categorical values, if you give it a numeric value you will get an error
...
weekdays(MplsStops$date)

MplsStops$weekday <- weekdays(MplsStops$date)

ggplot(MplsStops, aes(x=MplsStops$weekday)) +
  geom_bar(aes(fill=race)) +
  labs(title="Number of Stops by Weekday", x="Weekday", y="Stops", fill="Race")
...
max(MplsStops$date) - min(MplsStops$date)

whiteos <- MplsStops[MplsStops$race == "White",]
...
ggplot(MplsStops, aes(x=lat,y=long,color=race)) + geom_bin2d() + theme_minimal() + facet_wrap(~race) 

ggplot(MplsStops, aes(x=lat,y=long,color=race)) + geom_density2d() + theme_minimal() + facet_wrap(~race) 

ggplot() +
  geom_density_2d(mapping=aes(x=MplsStops$lat, y=MplsStops$long)) +
  geom_density_2d(mapping=aes(x=whiteos$lat, y=whiteos$long),color="black")
...
sum(na.omit(MplsStops$gender == "Unknown")) ####na.omit takes the NA values out of the command so R can understand how to run it
sum(na.omit(MplsStops$gender == "Male"))
sum(na.omit(MplsStops$gender == "Female"))

ggplot(MplsStops, aes(x=race)) +
  geom_bar() + 
  facet_wrap(~gender)
...
##Summary of stops by neighborhood, by gender

summary(MplsStops$neighborhood)
neighborrace <- na.omit(table(MplsStops$neighborhood, MplsStops$gender, MplsStops$race))

neighborrace <- as.data.frame(neighborrace)

names(neighborrace)

names(neighborrace) <- c("Neighborhood", "Gender", "Race", "Frequency")

ggplot(neighborrace, aes(x=Neighborhood,y=Frequency,color=Gender)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle=90))

pop = neighborrace[neighborrace$Frequency >500,]

pop

ggplot(pop, aes(x=Neighborhood,y=Frequency,fill=Gender)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle=90)) +
  facet_wrap(~Race)
...
#does poverty affect college graduation?
...
##practicing JPEG process

jpeg("../practiceplot.jpg")
hist(MplsStops$lat, xlab="Latitude", main="Latitude Histogram")
dev.off()
